...
(function($) {
    ...
    **/
    ...
    PickerHandler.prototype.destroy = function(){
        ...
        if ($picker.data("inputObjectId") != null) {
            InputObjects[$picker.data("inputObjectId")] = null;
        }
        // PickerObjects.splice(picker_id, 1);

        // remove all binding events & DOM nodes that newly generated
        var parent = $picker.parent();
        // events
        $("a, td, .datepicker_calendar", $picker).unbind();
        $picker.unbind();
        this.getInput().unbind();
        this.getInput().removeData('pickerId');
        // DOM nodes
        $picker.remove();
        parent.remove();
        this.$pickerObject = null;
    ...
    **/
    ...
    var outputToInputObject = function($picker) {
        ...
        var old = $inp.val();
        var newVal = getFormattedDate(date, format);

        if (! $picker.data('dateOnly')) {
            newVal += ":00";
        }

        $inp.val();
        if (old != $inp.val(newVal)) { // only trigger if it actually changed to avoid a nasty loop condition
            $inp.trigger("change");
        }
    };
    ...
    var draw = function($picker, option, year, month, day, hour, min) {
        ...
        if (isTodayButton) {
            ...
            $link_today.addClass('dp-icon-home');
        ...
        if (isCloseButton) {
            ...
            $link_close.addClass('dp-icon-close');
